...
let nombre = window.prompt("Escribe el nombre del que deseas los datos");

let img= personas.nombre.picture;
let name=nombre;
let edad=personas.nombre.age;
let email= personas.nombre.email;
let direccion=personas.nombre.address;
let colorDeOjos=personas.nombre.eyeColor;

   
if( nombre===personas.BridgetteMcGuire.name|| 
    nombre===personas.melindawheeler.name|| 
    nombre===personas.WardFulton.name|| 
    nombre===personas.TessaSellers.name|| 
    nombre===personas.KimberleyRoberson.name|| 
    nombre===personas.EleanorStafford.name|| 
    nombre===personas.CraneHarris.name
    
   ){

     
  let mostrarPersonas=`
  <div style= "background-color: ${colorDeOjos}"
  ...
  document.getElementById("div1").innerHTML=mostrarPersonas;
      ...
      document.getElementById("div1").innerHTML=`<p>El valor introducido no es correcto</p>`;
    }